...
// AuditStore implements snapshots database.
...
func (s *AuditStore) Log(ctx context.Context, entry *auditDomain.Entry) error {
	query := `INSERT INTO risk_audit (id, entity_type, entity_id, action, actor_id, old_value, new_value, ip_address, user_agent, timestamp)
