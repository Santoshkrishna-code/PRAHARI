...
import (
	...
	catDomain "prahari/services/observation/internal/domain/behaviorcategory"
)
...
func (s *CategoryStore) Create(ctx context.Context, c *catDomain.Category) error {
	query := `INSERT INTO behavior_categories (id, name, description) VALUES ($1, $2, $3)`
...
func (s *CategoryStore) FindByID(ctx context.Context, id string) (*catDomain.Category, error) {
	query := `SELECT id, name, description FROM behavior_categories WHERE id = $1`
