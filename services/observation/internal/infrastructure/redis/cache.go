...
func key(id string) string {
	return fmt.Sprintf("observation:detail:%s", id)
}

// Get returns cached record.
func (c *Cache) Get(ctx context.Context, id string) (*observationDomain.Observation, error) {
	data, err := c.client.Get(ctx, key(id))
	if err != nil {
		return nil, nil // Cache miss
	}

	var m observationDomain.Observation
...
// Set cache details.
func (c *Cache) Set(ctx context.Context, m *observationDomain.Observation) error {
