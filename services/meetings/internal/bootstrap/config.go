package bootstrap

import (
	"context"
	"fmt"

	"prahari/shared/config/sources"
)

type AppConfig struct {
	Port             int    `env:"PORT" envDefault:"8115"`
	Environment      string `env:"ENVIRONMENT" envDefault:"development"`
	DatabaseURL      string `env:"DATABASE_URL"`
	RedisAddr        string `env:"REDIS_ADDR" envDefault:"localhost:6379"`
	KafkaBrokers     string `env:"KAFKA_BROKERS" envDefault:"localhost:9092"`
	ShiftGrpcAddr    string `env:"SHIFT_GRPC_ADDR" envDefault:"localhost:8109"`
	PermitGrpcAddr   string `env:"PERMIT_GRPC_ADDR" envDefault:"localhost:8082"`
	IncidentGrpcAddr string `env:"INCIDENT_GRPC_ADDR" envDefault:"localhost:8081"`
	ActionGrpcAddr   string `env:"ACTION_GRPC_ADDR" envDefault:"localhost:8114"`
	TrainingGrpcAddr string `env:"TRAINING_GRPC_ADDR" envDefault:"localhost:8094"`
	S3Bucket         string `env:"S3_BUCKET" envDefault:"prahari-meetings-docs"`
	S3Region         string `env:"S3_REGION" envDefault:"ap-south-1"`
}

func LoadConfig(ctx context.Context) (*AppConfig, error) {
	var cfg AppConfig
	if err := sources.ParseEnv(&cfg); err != nil {
		return nil, fmt.Errorf("failed to load configuration: %w", err)
	}
	return &cfg, nil
}
