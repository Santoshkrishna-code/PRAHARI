...
import (
	...
	kafkaInfra "prahari/services/shift/internal/infrastructure/kafka"
	pgInfra "prahari/services/shift/internal/infrastructure/postgres"
	redisInfra "prahari/services/shift/internal/infrastructure/redis"

	// Interfaces
	eventsIface "prahari/services/shift/internal/interfaces/events"
	httpIface "prahari/services/shift/internal/interfaces/http"
)
...
func RunApp() {
	...
	tp, err := InitTelemetry(ctx, "shift-service", cfg.Environment)
	if err == nil && tp != nil {
	...
	handoverSvc := handoverApp.NewService(store, kafkaPublisher)
	logbookSvc := logbookApp.NewService(store)
	...
	handler := httpIface.NewHandler(schedulingSvc, handoverSvc, logbookSvc, reportingSvc, analyticsSvc, searchSvc, exportSvc)
	httpIface.RegisterRoutes(mux, handler)

	pipelineHandler := InitRouter(mux)
	srv := StartServer(cfg.Port, pipelineHandler)

	prahariLogger.Info(ctx, "Shift Management Service initialized successfully",
	...
	prahariLogger.Info(ctx, "Shutting down Shift Service...")

	shutdownCtx, shutdownCancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer shutdownCancel()

	_ = srv.Shutdown(shutdownCtx)
	prahariLogger.Info(ctx, "Shift Service exited gracefully.")
}
