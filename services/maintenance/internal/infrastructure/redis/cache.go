...
func key(id string) string {
	return fmt.Sprintf("maintenance:detail:%s", id)
}

// Get returns cached record.
func (c *Cache) Get(ctx context.Context, id string) (*maintenanceDomain.Maintenance, error) {
	data, err := c.client.Get(ctx, key(id))
	if err != nil {
		return nil, nil // Cache miss
	}

	var m maintenanceDomain.Maintenance
...
// Set cache details.
func (c *Cache) Set(ctx context.Context, m *maintenanceDomain.Maintenance) error {
