...
import (
	...
	pgInfra "prahari/services/energy/internal/infrastructure/postgres"
	redisInfra "prahari/services/energy/internal/infrastructure/redis"
	kafkaInfra "prahari/services/energy/internal/infrastructure/kafka"
	workflowInfra "prahari/services/energy/internal/infrastructure/workflow"
	eventsIface "prahari/services/energy/internal/interfaces/events"

	// Interfaces
	httpIface "prahari/services/energy/internal/interfaces/http"
)
...
func RunApp() {
	...
	tp, err := InitTelemetry(ctx, "energy-service", cfg.Environment)
	if err == nil && tp != nil {
	...
	// DI Injection
	sustSvc := reportingApp.NewService(store, kafkaPublisher, store, store)
	monSvc := consumptionApp.NewService(store, kafkaPublisher, store, store)
	carbonSvc := forecastingApp.NewService(store, kafkaPublisher, store, store)
	disclSvc := optimizationApp.NewService(store, kafkaPublisher, store, store)
	...
	prahariLogger.Info(ctx, "Energy Management Service initialized successfully",
	...
	prahariLogger.Info(ctx, "Shutting down Energy Management Service...")
	...
	prahariLogger.Info(ctx, "Energy Management Service exited gracefully.")
}
