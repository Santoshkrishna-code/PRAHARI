...
func key(id string) string {
	return fmt.Sprintf("contractor:detail:%s", id)
}

// Get returns cached record.
func (c *Cache) Get(ctx context.Context, id string) (*contractorDomain.Contractor, error) {
	data, err := c.client.Get(ctx, key(id))
	if err != nil {
		return nil, nil // Cache miss
	}

	var contractor contractorDomain.Contractor
	if err := json.Unmarshal([]byte(data), &contractor); err != nil {
		return nil, err
	}
...
// Set cache details.
func (c *Cache) Set(ctx context.Context, m *contractorDomain.Contractor) error {
