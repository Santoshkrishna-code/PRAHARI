...
import (
	...
	kafkaInfra "prahari/services/administration/internal/infrastructure/kafka"
	pgInfra "prahari/services/administration/internal/infrastructure/postgres"
	redisInfra "prahari/services/administration/internal/infrastructure/redis"

	// Interfaces
	eventsIface "prahari/services/administration/internal/interfaces/events"
	httpIface "prahari/services/administration/internal/interfaces/http"
)
...
func RunApp() {
	...
	// DI wiring
	tenantSvc := tenantApp.NewService(store, kafkaPublisher)
	hierarchySvc := hierarchyApp.NewService(store, kafkaPublisher)
	configurationSvc := configurationApp.NewService(store, kafkaPublisher)
	licensingSvc := licensingApp.NewService(store, kafkaPublisher)
	...
	handler := httpIface.NewHandler(tenantSvc, hierarchySvc, configurationSvc, licensingSvc, reportingSvc, analyticsSvc, searchSvc, exportSvc)
	...
	prahariLogger.Info(ctx, "Enterprise Administration Service initialized successfully",
	...
	prahariLogger.Info(ctx, "Shutting down Administration Service...")
	shutdownCtx, shutdownCancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer shutdownCancel()
	_ = srv.Shutdown(shutdownCtx)
	prahariLogger.Info(ctx, "Administration Service exited gracefully.")
}
